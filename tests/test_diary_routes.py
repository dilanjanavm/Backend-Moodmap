...
#TC010 - Get a specific diary entry by ID
def test_get_diary_entry_by_id(client):
    # Step 1: Perform a login to get a JWT token
    login_response = client.post('/login', json={'email': 'testuser@example.com', 'password': 'testpass'})
    assert login_response.status_code == 200
    ...
    # Step 3: Create a new diary entry for testing
    create_response = client.post('/predict_details', headers=headers, json={
        'text': 'I felt really happy today.',
        'selected_dairy_date': '2024-09-17'
    })
    assert create_response.status_code == 200
    created_entry_id = create_response.get_json()['data']['id']  # Extract the diary entry ID

    # Step 4: Retrieve the specific diary entry by ID
    response = client.get(f'/diary-reports/{created_entry_id}', headers=headers)
    ...
    data = response.get_json()
    assert 'data' in data
    assert data['data']['id'] == created_entry_id  # Ensure the correct entry is returned
    assert 'emotion_reports' in data['data']  # Ensure emotion reports are included
